...
| F-2-1 | Kept explicit `/#install` behavior: land on the section, focus its heading, announce it, and leave demo storage. | `@regression:install-navigation`, `@regression:install-deep-link`; [live install](https://selfhost-upgrade-rehearsal.sociobot.in/#install); final audit records `install-title` focus and section top. |
